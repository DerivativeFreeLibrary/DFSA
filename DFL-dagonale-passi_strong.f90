...
!			alfa_diag(1) = alfa_d(1)
!			do i = 2,n
!				alfa_diag(1) = alfa_diag(1) + alfa_d(i)
!			enddo
!			alfa_diag(1) = alfa_diag(1)/dble(n)
!			alfa_diag = alfa_diag(1)
!write(*,*) '    direzione'
	  if(n>1) then
...
100        format(' ni=',i4,'  nf=',i5,'   f=',d12.5,'   alfamax=',d12.5)
	...
	!	 write(*,*) 'alfa = ',alfa,' i_corr = ',i_corr
!		 write(1,*) 'alfa = ',alfa,' i_corr = ',i_corr
...
!		    if( (dot_product(flag_fail,flag_fail)==n).and.(maxval(alfa_d).le.soglia).and.(n>1) ) then
...
!!							   D1(I)=(XFSTOP(I,IMIN)-XFSTOP(I,IMAX))/(DOLDALFAMAX+DOLDALFAMIN)
!!							   D1(I)=(XFSTOP(I,IMIN)-XFSTOP(I,IMAX))/(DOLDALFAMAX)
							!  D1(I)=(XFSTOP(I,IMIN)-XFSTOP(I,IMAX))/DOLDALFAMIN
							!  D1(I)=(XFSTOP(I,IMIN)-XFSTOP(I,IMAX))/DOLDALFAMEDIO
							!  D1(I)=(X(I)-XFSTOP(I,IMAX))/DOLDALFAMAX
!							  D1(I)=X(I)-XFSTOP(I,IMAX)
							ELSE
!							   D1(I)=(X(I)-XFSTOP(I,IMAX))!/DOLDALFAMAX
							   D1(I)=(XFSTOP(I,IMIN)-XFSTOP(I,IMAX))
							   d1(I)=i
...
!  					index_sobol = index_sobol + 2*n
				    ...
				    dnr = dnr / dble(n)
			 	    ...
			 	    alfa_d =1.d+0*dnr			 	  !  alfa_d =1.d+0*dnr
				    !alfa_d = alfa_dense
...
!					soglia=soglia*.1d0
	     ...
	     call funct_vinc(n,mi,me,x,fob,constr)
 !        nf = nf+1		!forse non serve

!	     if(.false.) then
		 if((viol.gt.0.d0).and.(mi+me>=1)) then 		 
!		 if(((viol.gt.0.d0).and.(viol.gt.viol_old)).and.(m>=1)) then 
!            viol_old=viol
...
!     #######################################################
      ...
      !do i=1,n				 
      !    if(alfa_diag(i).gt.alfa_max) then
      !      alfa_max=alfa_diag(i)
	  !    end if
      !end do
